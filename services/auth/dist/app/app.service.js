...
const token_tool_1 = require("../helpers/tools/token.tool");
...
let AppService = class AppService {
    ...
    async login(user) {
        try {
            ...
            if (findUser.email != (user === null || user === void 0 ? void 0 : user.email)) {
                return (0, response_error_1.CustomError)(list_error_1.errorList.Not_Found_Error.message, list_error_1.errorList.Not_Found_Error.code);
            }
            const verifyPass = await new user_1.default(findUser).checkPassword(user.password);
            if (!verifyPass) {
                return (0, response_error_1.CustomError)(list_error_1.errorList.Unauthorized_Error.message, list_error_1.errorList.Unauthorized_Error.code);
            }
            ...
            const token = await new token_tool_1.default(userToken).NewToken();
            return { token };
        }
        catch (error) {
            console.log(error);
